...
Position.prototype.generateMoves = function(onlyCaptures) {
	...
	while (!turnQueens.isEmpty()) {
		...
		addNormalMoves(queenPosition, Position.makeBishopAttackMask(Chess.Bitboard.makeIndex(queenPosition), occupied).or(
			Position.makeRookAttackMask(Chess.Bitboard.makeIndex(queenPosition), occupied)).and(mask), Chess.Piece.QUEEN);
	}
	...
	while (!turnBishops.isEmpty()) {
		...
		addNormalMoves(bishopPosition, Position.makeBishopAttackMask(Chess.Bitboard.makeIndex(bishopPosition), occupied).and(mask), Chess.Piece.BISHOP);
	}
	...
	while (!turnRooks.isEmpty()) {
		...
		addNormalMoves(rookPosition, Position.makeRookAttackMask(Chess.Bitboard.makeIndex(rookPosition), occupied).and(mask), Chess.Piece.ROOK);
	}
